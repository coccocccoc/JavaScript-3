...
// 배열에 Apple이 포함되어 있는지 확인
let result = arr1.includes('Apple');
...
// 실제 값의 목록은 힙영역에 저장되어 있다
// 따라서 주소가 변경되지 않았기 때문에 목록을 변경할 수 있다

// 객체형 배열 만들기
const objArr = [];
// ctrl + space => 변수 목록을 보고 선택
objArr.push({ name: '둘리', age: '10' });
objArr.push({ name: '도우너', age: 15 });
console.log(objArr);

// 배열 안에 있는 요소 하나씩 꺼내기
console.log(objArr[0]); // { name: '둘리', age: '10' }
console.log(objArr[1]); // { name: '도우너', age: 15 }
